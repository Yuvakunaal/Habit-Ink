...
function InkPen3D({ scale = 1 }: { scale?: number }) {
  ...
  return (
    <div style={{ filter: "drop-shadow(6px 14px 28px rgba(43,58,140,0.32))", animation: "penFloat 3.8s ease-in-out infinite" }}>
      <svg width={w} height={h} viewBox="0 0 52 210" fill="none">
        <defs>
...
// ─── MODAL PEN ICON ──────────────────────────────────────────────────────────
// Diagonal fountain pen matching the reference image:
// dark brown lacquered body, warm-gold fan nib with slit + vent hole,
// gold clip and ring. Drawn horizontal in a 200×200 viewBox, rotated −40°.
  ...
  return (
    <div style={{ filter: "drop-shadow(4px 8px 20px rgba(0,0,0,0.26))" }}>
      <svg width="148" height="148" viewBox="0 0 200 200" fill="none">
        <defs>
          {/* Dark brown lacquer — cylindrical shading top→bottom */}
          ...
          {/* Gold ring + clip — warm amber with cylindrical sheen */}
          ...
          {/* Nib upper tine — bright face, light top → dark bottom */}
          ...
          {/* Nib lower tine — shadow face, dark throughout */}
        ...
        {/* Pen drawn horizontal with NIB on LEFT, CAP on RIGHT.
            rotate(-40) then maps: nib → lower-left, cap → upper-right.
            All x values are mirrored: new_x = 200 − old_x.            */}
        <g transform="rotate(-40, 100, 100)">

          {/* Ground shadow */}
          <ellipse cx="100" cy="117" rx="90" ry="5.5" fill="rgba(0,0,0,0.09)" />

          {/* ── Cap (rounded dome — now on RIGHT) ── */}
          <path d="M182 86 Q196 86 196 100 Q196 114 182 114 L92 114 L92 86 Z"
            fill="url(#mpBody)" />

          {/* ── Gold separation ring ── */}
          <rect x="84" y="84" width="9" height="32" rx="1.5" fill="url(#mpGold)" />
          <rect x="89" y="85.5" width="3.5" height="10" rx="1.75"
            fill="rgba(255,255,255,0.38)" />

          {/* ── Barrel ── */}
          <rect x="42" y="86" width="42" height="28" fill="url(#mpBody)" />

          {/* Cylindrical highlight stripe across barrel + cap */}
          <rect x="42" y="87.5" width="152" height="9" rx="4.5"
            fill="rgba(255,255,255,0.11)" />
          {/* Bottom reflected light */}
          <rect x="42" y="108" width="152" height="4" rx="2"
            fill="rgba(255,255,255,0.05)" />

          {/* ── Clip (runs along top of cap, ball toward barrel side) ── */}
          <rect x="101" y="83.5" width="75" height="5" rx="2.5" fill="url(#mpGold)" />
          <circle cx="101" cy="86" r="4.8" fill="url(#mpGold)" />
          <rect x="125" y="84.2" width="48" height="2.2" rx="1.1"
            fill="rgba(255,255,255,0.34)" />
          <circle cx="102.5" cy="85" r="1.8" fill="rgba(255,255,255,0.38)" />

          {/* ── Grip (narrow matte section — now on LEFT side of barrel) ── */}
          <rect x="28" y="88" width="14" height="24" rx="2.5" fill="url(#mpGrip)" />
          {[91, 94.5, 98, 101.5, 105, 108.5].map(y => (
            <line key={y} x1="28" y1={y} x2="42" y2={y}
              stroke="rgba(210,140,30,0.18)" strokeWidth="0.75" />
          ))}

          {/* ── Gold nib collar ── */}
          <rect x="21" y="85.5" width="7" height="29" rx="1.5" fill="url(#mpGold)" />
          <rect x="24.5" y="87" width="3" height="8.5" rx="1.5"
            fill="rgba(255,255,255,0.40)" />

          {/* ── Nib — fan shape tapering to point on LEFT ── */}
          {/* Upper tine (faces upward / toward viewer) */}
          <path d="M21 88 Q11 90 4 100 L21 100 Z" fill="url(#mpNibTop)" />
          {/* Lower tine (shadow side) */}
          <path d="M21 112 Q11 110 4 100 L21 100 Z" fill="url(#mpNibBot)" />
          {/* Center slit */}
          <line x1="21" y1="100" x2="4" y2="100"
            stroke="rgba(4,1,0,0.72)" strokeWidth="1.3" />
          {/* Vent / breather hole */}
          <ellipse cx="14" cy="100" rx="3.2" ry="3.2" fill="rgba(4,1,0,0.50)" />
          {/* Upper tine catch-light */}
          <path d="M21 89.5 Q14 89 8 95"
            stroke="rgba(255,210,90,0.35)" strokeWidth="1.2"
...
function Hero({ onOpenModal }: { onOpenModal: () => void }) {
  ...
  return (
    <section style={{ position: "relative", minHeight: "100vh", backgroundColor: BG, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      {/* Dot grid */}
      <div style={{ position: "absolute", inset: 0, pointerEvents: "none", opacity: 0.26, backgroundImage: `radial-gradient(circle, ${BORDER} 1px, transparent 1px)`, backgroundSize: "30px 30px" }} />
      ...
      {/* Corner rings */}
      <div style={{ position: "absolute", top: -80,  left: -80,  width: 240, height: 240, borderRadius: "50%", border: `2px dashed ${NAVY}`, opacity: 0.055, transform: "rotate(15deg)",  pointerEvents: "none" }} />
      <div style={{ position: "absolute", bottom: -100, right: -100, width: 320, height: 320, borderRadius: "50%", border: `2px dashed ${GOLD}`, opacity: 0.065, transform: "rotate(-20deg)", pointerEvents: "none" }} />
      <div style={{ position: "absolute", top: "35%",  right: "5.5%", width: 130, height: 130, borderRadius: "50%", border: `1.5px dashed ${GOLD}`, opacity: 0.045, pointerEvents: "none" }} />
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: isMobile ? "0 24px 60px" : "0 56px 80px", textAlign: "center", position: "relative", zIndex: 1 }}>
        ...
        {/* Social proof — FIXED: stacks cleanly on mobile */}
        <div style={{
          display: "flex",
          ...
          <span style={{ ...INTER, fontSize: 13, color: MUTED, textAlign: "center" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 14, justifyContent: "center", opacity: linesVisible ? 1 : 0, transition: "opacity 0.6s ease 1100ms" }}>
          <GoogleLogo size={14} />
          <span style={{ ...INTER, fontSize: 13, color: MUTED }}>Sign in with Google · Free forever · No credit card</span>
        </div>
      </div>
    </section>
  );
...
function HowItWorks() {
  ...
  return (
    <section style={{ backgroundColor: BG, padding: isMobile ? "72px 24px" : "96px 56px", borderTop: `1px solid ${BORDER}` }}>
      <p style={{ ...INTER, fontSize: 11, fontWeight: 600, letterSpacing: "0.18em", color: MUTED, textTransform: "uppercase", textAlign: "center", margin: "0 0 56px" }}>
        HOW IT WORKS
      </p>
      <div ref={ref} style={{ display: "flex", flexDirection: isMobile ? "column" : "row", gap: isMobile ? 44 : 0, maxWidth: 860, margin: "0 auto", position: "relative", alignItems: "flex-start" }}>
        {!isMobile && (
          <div style={{ position: "absolute", top: 28, left: "16.67%", right: "16.67%", borderTop: `2px dashed ${BORDER}`, zIndex: 0 }} />
        )}
        {HOW_STEPS.map((step, i) => (
          <div key={i} style={{ flex: 1, textAlign: "center", position: "relative", zIndex: 1, padding: isMobile ? "0 24px" : "0 28px", opacity: inView ? 1 : 0, transform: inView ? "translateY(0)" : "translateY(28px)", transition: `all 0.55s ease ${i * 160}ms` }}>
            {/* Step icon with glow */}
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, marginBottom: 18 }}>
              <div style={{ position: "relative" }}>
                <div style={{ position: "absolute", inset: -6, borderRadius: "50%", background: `radial-gradient(circle, ${step.color}22 0%, transparent 70%)` }} />
                <div style={{ width: 58, height: 58, borderRadius: "50%", backgroundColor: step.color, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26, boxShadow: `0 6px 22px ${step.color}44`, border: "3px solid #FAF8F3", position: "relative" }}>
                  {step.emoji}
                </div>
              </div>
              <span style={{ ...CAVEAT, fontSize: 12, fontWeight: 700, color: step.color, letterSpacing: "0.14em" }}>{step.num}</span>
            </div>
...
// ─── THREE TRUTHS ─────────────────────────────────────────────────────────────
const TRUTH_CARDS = [
  { emoji: "📅", heading: "Track",    body: "Every habit, every day. Check things off and build your streak.", accent: NAVY },
  { emoji: "📔", heading: "Journal",  body: "Capture your wins, your challenges, and your intentions.",         accent: GREEN },
  { emoji: "📊", heading: "Progress", body: "Charts, streaks, and insights that show how far you've come.",     accent: GOLD },
] as const;
...
function ThreeTruths() {
  ...
  return (
    <section style={{ backgroundColor: BG, padding: isMobile ? "80px 24px" : "100px 56px", textAlign: "center" }}>
        ...
        {TRUTH_CARDS.map((card, i) => {
          ...
          return (
            ...
            >
              ...
              <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 4, borderRadius: "22px 22px 0 0", backgroundColor: hot ? "rgba(255,255,255,0.18)" : card.accent, transition: "background-color 0.28s" }} />

              <div style={{ width: 58, height: 58, borderRadius: "50%", backgroundColor: hot ? "rgba(255,255,255,0.12)" : `${card.accent}15`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, margin: "12px auto 20px", transition: "background-color 0.28s" }}>
                {card.emoji}
              </div>
...
function HabitMockup() {
  ...
  return (
    <div style={{ backgroundColor: "#fff", border: `1px solid ${BORDER}`, borderRadius: 20, padding: "22px 22px", width: 252, flexShrink: 0, boxShadow: "0 12px 44px rgba(0,0,0,0.14)", transform: "rotate(2.5deg)" }}>
      ...
      {items.map((item, i) => (
        <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 0", borderBottom: i < items.length - 1 ? `1px solid ${BORDER}` : "none" }}>
          ...
          {item.done
            ...
            : <div style={{ width: 22, height: 22, borderRadius: "50%", border: "2px solid #CCC8BF" }} />
          }
        </div>
      ))}
...
function ScreensInFocus() {
  ...
  return (
    <section style={{ backgroundColor: BG, padding: isMobile ? "60px 24px" : "80px 56px" }}>
...
function StreakProof() {
  ...
  return (
    <section style={{ backgroundColor: BG, padding: isMobile ? "80px 24px" : "100px 56px", textAlign: "center", borderTop: `1px solid ${BORDER}` }}>
      ...
      <div ref={ref} style={{ display: "flex", flexWrap: "wrap", gap: isMobile ? 6 : 8, justifyContent: "center", maxWidth: isMobile ? 280 : 490, margin: "0 auto 22px" }}>
...
function QuoteMoment() {
  ...
  return (
    <section style={{ backgroundColor: BG, padding: isMobile ? "80px 24px" : "120px 56px", textAlign: "center", position: "relative", overflow: "hidden" }}>
      ...
      <p style={{ ...INTER, fontSize: 11, fontWeight: 600, letterSpacing: "0.18em", color: NAVY, textTransform: "uppercase", margin: "0 0 40px", position: "relative" }}>
        TODAY'S REFLECTION
      </p>
      <blockquote
        ref={ref}
...
function DarkClose({ onOpenModal }: { onOpenModal: () => void }) {
  ...
  return (
    <section style={{
      ...
      {STAR_DOTS.map((d, i) => (
        <div key={i} style={{ position: "absolute", top: d.top, ...(d.left != null ? { left: d.left } : { right: d.right }), width: d.size, height: d.size, borderRadius: "50%", backgroundColor: BG, opacity: d.opacity, pointerEvents: "none" }} />
      ))}
      {/* Central glow */}
      <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", width: 500, height: 500, borderRadius: "50%", background: "radial-gradient(circle, rgba(201,168,76,0.08) 0%, transparent 68%)", pointerEvents: "none" }} />
      {/* Top edge mini pen accent */}
      <div style={{ position: "absolute", top: 48, right: isMobile ? "8%" : "12%", opacity: 0.25, pointerEvents: "none" }}>
        <InkPen3D scale={0.5} />
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6, justifyContent: "center", marginBottom: 60 }}>
        <GoogleLogo size={13} />
        <span style={{ ...INTER, fontSize: 12, color: "rgba(250,248,243,0.38)" }}>Free. Private. No spam. Ever.</span>
      </div>
